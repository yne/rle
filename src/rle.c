	...
	while (fpdu && sdu) {
		...
		**/
		if(fpdu->size == 0){
			...
			fpdu->size += profile->use_explicit_payload_header_map?1:0;
		}
		...
		*/
		...
		if (sdu->_.is_frag == false) { /* SDU not (yet) fragged, because it's it first pass: find out if we can fit a COMP, or a START, (or nothing) */
			...
			if(sizeof(ppdu_cont_header_t) + ppdu_label_size <= fpdu_remaining_size){
				...
				mempush(fpdu->data, &((ppdu_cont_header_t){0,0, alpdu_avail + sizeof(ppdu_cont_header_t) + ppdu_label_size, sdu->fragment_id}), sizeof(ppdu_cont_header_t),fpdu->size);
				mempush(fpdu->data, ppdu_label_byte, ppdu_label_size,fpdu->size);
...
int rle_decap(rle_profile* profile, rle_fpdu_iter next_fpdu, rle_sdu_iter next_sdu){
	rle_fpdu_t*fpdu = next_fpdu();
	size_t fpdu_offset = RLE_FPDU_LABEL_SIZE+profile->use_explicit_payload_header_map?sizeof(fpdu_header_t):0;
	rle_sdu_t*sdu = next_sdu();
	while (fpdu && sdu) {
		...
		if(ppdu_hdr->start_indicator && ppdu_hdr->end_indicator){
			printf("LT:%i T:%i\n",sdu->label_type = (ppdu_hdr->frag_id & 4), sdu->_.ptype_suppr = (ppdu_hdr->frag_id & 3));
			sdu->label_size = profile->alpdu_label_size[sdu->label_type];
		}else{
			printf("ID:%i\n",ppdu_hdr->frag_id);
		}
		...
		fpdu = next_fpdu();
		fpdu_offset = RLE_FPDU_LABEL_SIZE+profile->use_explicit_payload_header_map?sizeof(fpdu_header_t):0;
	}
	/*
